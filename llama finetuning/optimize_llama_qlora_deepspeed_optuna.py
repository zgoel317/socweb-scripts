...
def setup_distributed():
    ...
    # Initialize torch.distributed process group if not already initialized
    import torch.distributed as dist
    if not dist.is_initialized():
        dist.init_process_group(backend="nccl", init_method="env://")
        debug_print("Initialized torch.distributed process group", important=True)
...
def create_model_and_tokenizer(model_name=None, num_labels=2):
    ...
    try:
        ...
        # Load model with BF16 quantization (OPTIMIZATION 4: Switch to BF16 for L40S)
        debug_print("Loading model with BF16 quantization...", important=True)
        ...
        model = AutoModelForSequenceClassification.from_pretrained(
            ...
            torch_dtype=torch.bfloat16,  # OPTIMIZATION 4: Use BF16 instead of FP16
            device_map=None,  # Disable automatic device mapping - let DeepSpeed handle sharding
            quantization_config=BitsAndBytesConfig(
                load_in_8bit=True,
                llm_int8_threshold=6.0,
                bnb_4bit_compute_dtype=torch.bfloat16,  # OPTIMIZATION 4: Use BF16 for compute
            )
        )
...
MODEL_PATH = "put your local model path here"  # Replace with your actual model path
...
def create_deepspeed_config():
    ...
    ds_config = {
        ...
        "bf16": {  # OPTIMIZATION 4: Switch to BF16 for L40S
            "enabled": True,
        },
        "fp16": {  # Keep FP16 as fallback but prefer BF16
            "enabled": False,
        },
        "optimizer": {
            "type": "AdamW",
...
def hp_space(trial):
    ...
    return {
        "learning_rate": trial.suggest_categorical("learning_rate", [1e-4, 2e-4, 3e-4, 5e-4]),  # Added higher learning rate
        "weight_decay": trial.suggest_categorical("weight_decay", [0.01, 0.05, 0.1]),  # Added more weight decay options
        ...
        "num_train_epochs": trial.suggest_categorical("num_train_epochs", [1, 2]),  # Added option for 3 epochs
        "warmup_ratio": trial.suggest_categorical("warmup_ratio", [0.03, 0.05, 0.1]),  # Added higher warmup ratio
        "early_stopping_patience": trial.suggest_categorical("early_stopping_patience", [1]),  # OPTIMIZATION 1: Reduced to 1 for more aggressive early stopping
...
def run_single_trial(hyperparams, tokenizer, train_dataset, val_dataset, trial_num):
    ...
    try:
        ...
        required_keys = ["learning_rate", "weight_decay", "per_device_train_batch_size", "lora_r", 
                        "gradient_accumulation_steps", "num_train_epochs", "warmup_ratio", "early_stopping_patience", "probability_decision_boundary"]
        ...
        # Calculate evaluation steps for less frequent evaluation (OPTIMIZATION 2)
        total_steps_per_epoch = len(train_dataset) // hyperparams["per_device_train_batch_size"]
        eval_steps = max(1, total_steps_per_epoch // 4)  # Evaluate 4 times per epoch instead of every epoch
        ...
        training_args = TrainingArguments(
            ...
            eval_strategy="steps",  # OPTIMIZATION 2: Evaluate by steps instead of epochs
            eval_steps=eval_steps,  # OPTIMIZATION 2: Less frequent evaluation
            save_strategy="no",  # OPTIMIZATION 1: Don't save checkpoints to save time
            load_best_model_at_end=False,  # OPTIMIZATION 1: Don't load best model since we're not saving
            ...
            bf16=True,  # OPTIMIZATION 4: Enable BF16
            fp16=False,  # OPTIMIZATION 4: Disable FP16
            local_rank=get_local_rank(),
            dataloader_num_workers=2,  # OPTIMIZATION 3: Enable 2 workers for CPU-GPU overlap
        ...
        # Create trainer with custom loss function and more aggressive early stopping (OPTIMIZATION 1)
        trainer = QLoRATrainer(
            ...
            callbacks=[
                EarlyStoppingCallback(
                    early_stopping_patience=hyperparams.get("early_stopping_patience", 1),  # OPTIMIZATION 1: Reduced to 1
                    early_stopping_threshold=0.01  # OPTIMIZATION 1: Only stop if F1 doesn't improve by at least 0.01
                )
            ]
        )
...
def run_optuna_optimization(cluster_labeling=False):
    """Run Optuna optimization with DeepSpeed - All ranks stay alive together"""
    ...
    try:
        ...
        # OPTIMIZATION 2: Check if pre-tokenized datasets exist
        import os  # Add missing import
        tokenized_dir = "tokenized_datasets"
        train_dataset_path = os.path.join(tokenized_dir, "train")
        val_dataset_path = os.path.join(tokenized_dir, "val")
        
        if os.path.exists(train_dataset_path) and os.path.exists(val_dataset_path):
            debug_print("Loading pre-tokenized datasets...", important=True)
            train_dataset = Dataset.load_from_disk(train_dataset_path)
            val_dataset = Dataset.load_from_disk(val_dataset_path)
            debug_print("Pre-tokenized datasets loaded successfully", important=True)
        else:
            debug_print("Creating and tokenizing datasets...", important=True)
            # Create datasets
            ...
            # OPTIMIZATION 2: Save tokenized datasets for reuse
            if is_main_process():
                debug_print("Saving tokenized datasets for future reuse...", important=True)
                os.makedirs(tokenized_dir, exist_ok=True)
                train_dataset.save_to_disk(train_dataset_path)
                val_dataset.save_to_disk(val_dataset_path)
                debug_print("Tokenized datasets saved successfully", important=True)
        ...
        # Create a shared Optuna Study (only on main process, but shared via SQLite)
        if is_main_process():
            debug_print("Main process creating Optuna study...", important=True)
            ...
            if os.path.exists(db_file):
                os.remove(db_file)
                debug_print(f"Deleted existing database file: {db_file}", important=True)
            
            # Create Optuna study with SQLite backend for sharing
            study_name = f"llama_qlora_optimization_{'cluster' if cluster_labeling else 'binary'}"
            study = optuna.create_study(
                direction="maximize",
                study_name=study_name,
                storage="sqlite:///optuna_study.db",  # file-based DB everyone can see
                load_if_exists=False,  # Don't load existing study - start fresh
                sampler=optuna.samplers.TPESampler(seed=42),
                pruner=optuna.pruners.MedianPruner(
                    n_startup_trials=3,  # OPTIMIZATION 1: Reduced from 5 to 3
                    n_warmup_steps=1,  # OPTIMIZATION 1: Reduced from 10 to 1 for faster pruning
                    interval_steps=1
                )
            )
            
            # Set number of trials (reduced for faster optimization)
            n_trials = 20  # Reduced from grid search to focus on promising regions
            
            debug_print(f"Starting Optuna optimization with {n_trials} trials...", important=True)
            print(f"Optuna optimization will run {n_trials} trials")
        else:
            n_trials = 20  # Non-main processes need to know the number of trials
        
        # Broadcast number of trials to all processes
        if torch.distributed.is_initialized():
            n_trials_tensor = torch.tensor(n_trials, dtype=torch.int32, device=get_current_device())
            torch.distributed.broadcast(n_trials_tensor, src=0)
            n_trials = n_trials_tensor.item()
            debug_print(f"[Rank {get_rank()}] Received n_trials: {n_trials}", important=True)
        
        # All ranks participate in the trial loop together
        for trial_idx in range(n_trials):
            debug_print(f"[Rank {get_rank()}] Starting trial {trial_idx}/{n_trials}...", important=True)
            
            # Rank 0 asks Optuna for the next hyperparameters
            if is_main_process():
                trial = study.ask()
                # Actually sample all of the hyperparameters into the trial
                hyperparams = hp_space(trial)
                debug_print(f"[Rank {get_rank()}] [Trial {trial_idx}] Generated hyperparameters: {hyperparams}", important=True)
                debug_print(f"[Rank {get_rank()}] [Trial {trial_idx}] Hyperparameters type: {type(hyperparams)}", important=True)
                debug_print(f"[Rank {get_rank()}] [Trial {trial_idx}] Hyperparameters keys: {list(hyperparams.keys()) if hyperparams else 'None'}", important=True)
            else:
                hyperparams = None
            
            # Broadcast hyperparameters to all ranks using broadcast_object_list
            if torch.distributed.is_initialized():
                # Rank 0 has hyperparams; others start with None
                params_list = [hyperparams] if is_main_process() else [None]
                
                # Broadcast the single-element list in-place
                torch.distributed.broadcast_object_list(params_list, src=0)
                hyperparams = params_list[0]
                
                if not is_main_process():
                    debug_print(f"[Rank {get_rank()}] [Trial {trial_idx}] Received hyperparameters: {hyperparams}", important=True)
                    debug_print(f"[Rank {get_rank()}] [Trial {trial_idx}] Hyperparameters type: {type(hyperparams)}", important=True)
                    debug_print(f"[Rank {get_rank()}] [Trial {trial_idx}] Hyperparameters keys: {list(hyperparams.keys()) if hyperparams else 'None'}", important=True)
            
            # Barrier so everyone starts the trial at once
            if torch.distributed.is_initialized():
                debug_print(f"[Rank {get_rank()}] [Trial {trial_idx}] Waiting for trial start barrier...")
                torch.distributed.barrier()
                debug_print(f"[Rank {get_rank()}] [Trial {trial_idx}] Trial start barrier passed")
            
            # Run the DDP/DeepSpeed training on all ranks
            best_metric = run_single_trial(hyperparams, tokenizer, train_dataset, val_dataset, trial_idx)
            
            # Barrier so everyone finishes up before the next trial
            if torch.distributed.is_initialized():
                debug_print(f"[Rank {get_rank()}] [Trial {trial_idx}] Waiting for trial completion barrier...")
                torch.distributed.barrier()
                debug_print(f"[Rank {get_rank()}] [Trial {trial_idx}] Trial completion barrier passed")
            
            # Rank 0 tells Optuna the result
            if is_main_process():
                study.tell(trial, best_metric)
                debug_print(f"[Rank {get_rank()}] [Trial {trial_idx}] Reported result to Optuna: {best_metric}", important=True)
            
            # Synchronize before next trial
            if torch.distributed.is_initialized():
                torch.distributed.barrier()
        
        # Get best results (only on main process)
        if is_main_process():
            debug_print("Optuna optimization completed", important=True)
            ...
            save_best_hyperparameters(study, "cluster" if cluster_labeling else "binary")
        ...
        debug_print(f"[Rank {get_rank()}] Optimization completed with best value: {best_value}", important=True)
        return best_value
