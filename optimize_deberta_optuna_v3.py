...
def tokenize(batch):
    return tokenizer([str(x) for x in batch["DM"]], padding="max_length", truncation=True, max_length=128)
...
class BinaryClassificationTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # For 2-class classification, we need to handle the logits properly
        if len(logits.shape) == 1:
            logits = logits.unsqueeze(0)
        
        # Convert binary labels (0,1) to indices (0,1)
        label_indices = labels.long()
        loss_fct = nn.CrossEntropyLoss()
        loss = loss_fct(logits, label_indices)
...
def compute_metrics(eval_pred):
    ...
    # For 2-class classification, get probability of class 1 (lonely)
    probs = torch.softmax(torch.tensor(logits), dim=-1).numpy()
    # prob_lonely = probability of class 1 (second column)
    prob_lonely = probs[:, 1]  # Probability of being lonely
    preds = (prob_lonely > 0.5).astype(int)  # Threshold at 0.5
...
def model_init():
    ...
    config.num_labels = 2  # Changed from 1 to 2 to match the checkpoint
    config.hidden_dropout_prob = current_trial_hp["dropout"]
    return AutoModelForSequenceClassification.from_pretrained(MODEL_PATH, config=config, ignore_mismatched_sizes=True)
...
def hp_space(trial):
    return {
        ...
        "num_train_epochs": trial.suggest_int("num_train_epochs", 5, 20),  # Now tuning epochs
...
def objective(trial, study=None, cluster_labeling=False):
    ...
    output_dir = f"./tuned_model_deberta_trial_{trial.number}{model_suffix}"
...
def train_final_model(best_hyperparams, cluster_labeling=False):
    ...
    # Create output directory for final model with labeling method
    model_suffix = "_cluster" if cluster_labeling else "_binary"
    output_dir = f"./final_deberta_model{model_suffix}"
    os.makedirs(output_dir, exist_ok=True)
    ...
    training_args = TrainingArguments(
        ...
        save_strategy="epoch",
        save_total_limit=3,
        logging_dir="./logs_final_deberta",
        logging_steps=50,
        report_to="none",
        fp16=torch.cuda.is_available(),
        dataloader_pin_memory=True,
    ...
    # Clean up all checkpoints after final training (only if not in distributed training)
    if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
        checkpoint_dirs = glob.glob(os.path.join(output_dir, "checkpoint-*"))
        for checkpoint in checkpoint_dirs:
            try:
    ...
    # Save final model
    final_model_path = f"{output_dir}_best"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='DeBERTa hyperparameter optimization with cluster labeling option')
    ...
    lonely_df = pd.read_csv("lonely_dms_0.5_0.7.csv")
    not_lonely_df1 = pd.read_csv("serious_dms_0.7_0.7.csv")
    not_lonely_df2 = pd.read_csv("casual_dms_0.7_0.7.csv")
    ...
    MODEL_PATH = "./deberta-v3-sst2-final"
    ...
    results_csv = "optuna_trials_log_deberta_v3.csv"
